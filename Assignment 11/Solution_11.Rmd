...
library(readr)
marketing.data = read_delim("marketing_campaigns.csv",col_names=T,show_col_types =FALSE)

marketing.data <- marketing.data %>%
  mutate(CTR = Clicks/Impressions)

CTR_filter <- marketing.data %>%
  group_by(Campaign) %>%
  mutate(avg_CTR = mean(CTR)) %>%
  filter(avg_CTR > 0.02)


CTR_filter$Date = floor_date(CTR_filter$Date, unit = "month")

monthly_summary = CTR_filter %>%
...
prod.data = read_delim("products.csv",show_col_types = F)

prod.data <- prod.data %>%
  mutate(Margin = (Retail.Price - Unit.Cost) / Retail.Price)

prod.data = prod.data %>% separate(Item, into =c('Item','Category'),sep ='- ')

unique_items <- unique(prod.data$Category)
 
prod.data$Simplified.Category <- fct_collapse(
  prod.data$Category,
...
write_csv(prod.data,"avg_margin_by_new_category.csv")
```
