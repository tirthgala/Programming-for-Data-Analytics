...
data.marketing =read_delim("marketing_campaigns.csv",col_names=T,show_col_types =FALSE)
```
...
data.marketing = mutate(data.marketing, CTR=Clicks/Impressions)
```
...
result.data = data.marketing %>% group_by(Campaign) %>% summarize(avg_CTR = mean(CTR)) %>% filter(avg_CTR > 0.02)
result.data
```
...
```{r problem2a}
# TODO: Load the data
data.product = read_delim("products.csv",col_names=T,show_col_types =FALSE)
...
data.product = mutate(data.product, Margin = (Retail.Price - Unit.Cost) / Retail.Price)
data.product
```
...
data.product = data.product %>% separate(Item, into = c("Item", "Category"), sep = " - ")
data.product
```
...
avg.margin = data.product %>% group_by(Category) %>% summarize(Avg_Margin = round(mean(Margin),15))

write.csv(avg.margin,"avg_margin_by_category.csv",row.names =FALSE)
```
